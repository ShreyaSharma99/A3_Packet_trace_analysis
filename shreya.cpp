...
public class ReadCSV{
	...
	public boolean check_out_of_order(long s , ArrayList<Packet> p){
		for(int i=0 ;i<p.size(); i++){
			if(Long.parseLong(p.get(i).getSeq())>(s)){
				System.out.println(p.get(i).getSeq() + " is > " + s);
				return true;
				// break;
			}
		}
			...
			if(data[4].equals("TCP")){
				if(mssg[3].equals("[SYN]")){
					...
					if(ind2>=0){
						...
						// TCP_connect tcp = 	live_TCP.get(ind2);
						if(pack_sent.size()>0){	
							String ack_last="";
							int last = pack_sent.size()-1;
							while(ack_last.length()==0 && last>=0){
								ack_last = pack_sent.get(last).getAck();
								last--;
							}
							if(ack_last.length()!=0){
							
								if(repeat_syn(find_seq(mssg),live_TCP.get(ind2).getPack_rec()) && find_seq(mssg).length()>0){
									System.out.println(Long.toString(Long.parseLong(find_seq(mssg))+1));
									// if(check_dup_ack(Long.toString(Long.parseLong(find_seq(mssg))+1),pack_sent))
									if(Long.parseLong(ack_last) > Long.parseLong(find_seq(mssg))){
										System.out.println(ack_last+ " ack < " + find_seq(mssg));
								...
								if(Long.parseLong(ack_last) < Long.parseLong(find_seq(mssg))){
										// System.out.println("ack asked: "+pack_sent.get(pack_sent.size()-1).getAck());
										// System.out.println("seq received: "+find_seq(mssg));
					...
					if(ind2>=0){
						...
						if(pack_sent.size()>0){
							...
							// if(check_out_of_order(Long.parseLong(find_seq(mssg)), live_TCP.get(ind2).getPack_rec())){
								if(Long.parseLong(ack_last) < Long.parseLong(find_seq(mssg))){
									live_TCP.get(ind2).setOut_of_order(true);
								}
							}
    ...
    { 
        ...
        System.out.println(TCP_connections.size()+Live_connections.size());
      ...
      try{
        ...
        // TCP_connect temp = TCP_list.get(0);
        // ArrayList<Packet> p = new ArrayList<Packet>();
        // for(int i=0; i<TCP_list.size(); i++ ){
        // 	p = temp.getPack_rec();
        // 	old = 0;
        // 	for(int j=0; j<p.size(); j++){
        // 		if(Long.parseLong(p.get(j).getSeq())<old)
        // 			System.out.println(i);
        // 		fw1.write(p.get(j).getTime()+"\n");
        // 		fw2.write(p.get(j).getSeq()+"\n");
        // 		old = Long.parseLong(p.get(j).getSeq());
        // 	}
        // 	if(i+1<TCP_list.size())
        // 		temp = TCP_list.get(i+1); 
        // }


        int max1= TCP_list.get(0).pack_sent.size() + TCP_connections.get(0).pack_rec.size();
        int max2 = TCP_connections.get(0).pack_sent.size() + TCP_connections.get(0).pack_rec.size();
        for(int i=0; i<TCP_connections.size();i++){
        	//System.out.println(TCP_connections.get(i).pack_rec.size());
        	int tmp = TCP_connections.get(i).pack_sent.size() + TCP_connections.get(i).pack_rec.size();
        	if(tmp > max1){
        		max2 = max1;
        		max1 = tmp;
        		maxind2 = maxind1;
        	...
        	ConnectionID temp = TCP_connections.get(i).getConnectionID();
        	// answer5 += temp.getSrcIP() + " " + temp.getDestIP()+ " " + temp.getSrcPort() + " " + temp.getDestPort();
        	if(i != TCP_connections.size()-1){
        		System.out.println(Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())));
        		fw1.write(Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()) + "\n");
        		fw2.write(Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())) +"\n");
        		fw3.write(Integer.toString(TCP_connections.get(i).getBytes_rec()- (20*TCP_connections.get(i).pack_rec.size())) +"\n");
        		// c1 += Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()) + "\n";
        		// c2 += Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())) +"\n";
        		// c3 += Integer.toString(TCP_connections.get(i).getBytes_rec()- (20*TCP_connections.get(i).pack_rec.size())) +"\n";
        	}
        		
        		// answer5 += " " + Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()) + " " + Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())) + " " + Integer.toString(TCP_connections.get(i).getBytes_rec()- (20*TCP_connections.get(i).pack_rec.size())) +"\n";
        	else{
        		System.out.println(Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()) );
        		fw1.write(Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()));
        		fw2.write(Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())));
        		fw3.write(Integer.toString(TCP_connections.get(i).getBytes_rec()- (20*TCP_connections.get(i).pack_rec.size())));
        		// answer5 += " " + Float.toString(TCP_connections.get(i).getT_end() - TCP_connections.get(i).getT_start()) + " " + Integer.toString(TCP_connections.get(i).getBytes_sent() - (20*TCP_connections.get(i).pack_sent.size())) + " " + Integer.toString(TCP_connections.get(i).getBytes_rec()- (20*TCP_connections.get(i).pack_rec.size()));
  ...
  //   	int ind1 = -1;
  //   	int ind2 = -1;
		// System.out.println("Q 9.c");
		// for(int i=0; i<TCP_connections.size(); i++){
		// 	if(TCP_connections.get(i).spur_retransmit && i!=maxind1){
		// 		// System.out.println(i);
		// 		ind1 = i;
		// 		break;
		// 	}	
		...
		// 	if(TCP_connections.get(i).spur_retransmit && i!=maxind2){
		// 		// System.out.println(i);
		// 		ind2 = i;
		// 		break;
		// 	}	
  ...
  //   	if(ind1>=0 && ind2>=0){
	 ...
	 //        try{
		//       	FileWriter fw=new FileWriter("q9c_21_rec.txt");    
	 ...
	 //      	}catch(Exception e){
		...
		// 	try{
		//       	FileWriter fw=new FileWriter("q9c_21_sent.txt");    
	 ...
	 //        try{
		//       	FileWriter fw=new FileWriter("q9c_22_rec.txt");    
		//         fw.write(s_packet_rec);    
		//         fw.close();  
	 //      	}catch(Exception e){
		//       	System.out.println(e);
		...
		// 	try{
		//       	FileWriter fw=new FileWriter("q9c_22_sent.txt");    
		//         fw.write(s_packet_ack_sent);    
		//         fw.close();  
	 //      	}catch(Exception e){
		//       	System.out.println(e);
...
//       ArrayList<TCP_connect> TCPs = reader.getTCP_connections();
